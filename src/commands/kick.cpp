...
namespace s = bot::strings::cmd::kick;

class KickCommand final : public bot::Command {
public:
    std::string_view name() const noexcept override { return "kick"; }

    dpp::slashcommand definition(dpp::snowflake bot_id) const override {
        return dpp::slashcommand("kick", s::description, bot_id)
    ...
    void execute(const dpp::slashcommand_t& event) const override {
        if (!event.command.member.permissions.has(dpp::p_kick_members)) {
        ...
        event.from->creator->set_audit_reason(reason)
            .guild_member_kick(event.command.guild_id, target, [event, target](const dpp::confirmation_callback_t& cb) {
                if (cb.is_error()) {
